...
export class LandingPage extends BaseApi {
  ...
  render () {
    ...
    main.appendChild(yo`
      <span style="padding-left: 60px; "onclick="${() => { globalRegistry.get('verticalicon').api.select('pluginManager') }}">
        Check out all plugins here 
        <img class="btn btn-secondary btn-lg m-2" style="max-width: 53px; max-height: 53px" src="${icon}" />
      </span>
    `)
