  ...
  runOrCallContractMethod (contractName, contractABI, funABI, value, address, params, lookupOnly, logMsg, logCallback, outputCb, callbacksInContext) {
    ...
    txFormat.buildData(contractName, contractABI, {}, false, funABI, params, (error, data) => {
      if (!error) {
        ...
        this.udapp.callFunction(address, data, funABI, callbacksInContext.confirmationCb.bind(callbacksInContext), callbacksInContext.continueCb.bind(callbacksInContext), callbacksInContext.promptCb.bind(callbacksInContext), (error, txResult) => {
          if (!error) {
            var isVM = this.executionContext.isVM()
            if (isVM) {
              var vmError = txExecution.checkVMError(txResult)
      ...
      // called for libraries deployment
      this.udapp.runTx(data, callbacksInContext.confirmationCb.bind(callbacksInContext), runTxCallback)
    })
  }
