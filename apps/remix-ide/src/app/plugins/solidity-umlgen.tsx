...
export class SolidityUmlGen extends ViewPlugin implements ISolidityUmlGen {
    ...
    this.on(
      ...
      async (file: string, source, languageVersion, data, input, version) => {
        ...
        try {
          ...
          let umlDot = ''
          this.activeTheme = themeCollection.find(
            (theme) => theme.themeName === currentTheme.name
          )
  ...
  private handleThemeChange() {
    this.on('theme', 'themeChanged', async (theme) => {
      ...
      const themeQuality: ThemeQualityType = await this.call(
        'theme',
        'currentTheme'
      )
  ...
  async mangleSvgPayload(svgPayload: string): Promise<string> {
    ...
    themeCollection.forEach((theme) => {
      if (theme.themeName === themeQuality.name) {
        parsedDocument.documentElement.setAttribute(
          'style',
          `background-color: var(${this.getThemeCssVariables('--body-bg')})`
        )
