...
TESTFILES=$(grep -IRiL "\'@disabled\': \?true" "dist/apps/remix-ide-e2e/src/tests" | grep "\.nightly" | sort )

# count test files
fileCount=$(grep -IRiL "\'@disabled\': \?true" "dist/apps/remix-ide-e2e/src/tests" | grep "\.nightly" | wc -l )
# if fileCount is 0
if [ $fileCount -eq 0 ]
then
  echo "No nightly tests found"
  exit 0
fi
