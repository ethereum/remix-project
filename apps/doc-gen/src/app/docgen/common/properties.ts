...
export function signature({ item }: DocItemContext): string | undefined {
  ...
  case 'FunctionDefinition': {
    ...
    const res = [`${head}(${params.map(formatVariable).join(', ')})`, item.visibility]
...
export function items({ item }: DocItemContext): DocItem[] | undefined {
  return item.nodeType === 'ContractDefinition' ? item.nodes.filter(isNodeType(docItemTypes)).filter((n) => !('visibility' in n) || n.visibility !== 'private') : undefined
}
...
export function variables({ item }: DocItemContext): VariableDeclaration[] | undefined {
  return item.nodeType === 'ContractDefinition' ? item.nodes.filter(isNodeType('VariableDeclaration')).filter((v) => v.stateVariable && v.visibility !== 'private') : undefined
}
...
export function inheritance({ item, build }: DocItemContext) {
  ...
  return item.linearizedBaseContracts.map((id) => build.deref('ContractDefinition', id)).filter((c, i) => c.name !== 'Context' || i === 0)
}
