...
export const pageAssigner: Record<PageStructure & string, PageAssigner> = {
  ...
  files: (_, file, { pageExtension: ext, sourcesDir }) => path.relative(sourcesDir, file.absolutePath).replace('.sol', ext)
}
...
function assignIfIncludedSource(assign: PageAssigner, item: DocItem, file: SourceUnit, config: SiteConfig) {
