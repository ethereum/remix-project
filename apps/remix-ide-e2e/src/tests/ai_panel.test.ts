...
module.exports = {
  ...
  'Should generate new workspace contract code with the AI assistant': function (browser: NightwatchBrowser) {
    browser
      .waitForElementVisible('*[data-id="composer-textarea"]')
      .assistantGenerate('a simple storage contract', 'mistralai')
      .waitForElementVisible({
        locateStrategy: 'xpath',
  ...
  'Should lead to Workspace generation with the AI assistant': function (browser: NightwatchBrowser) {
    browser
      .waitForElementVisible('*[data-id="composer-textarea"]')
      .assistantWorkspace('comment each function', 'mistralai')
      .waitForElementVisible({
        locateStrategy: 'xpath',
        selector: '//*[@data-id="remix-ai-assistant" and contains(.,"Modified Files")]'
      })
      .pause(5000)
  },
  ...
  //   browser
  //     .waitForElementVisible('*[data-id="composer-textarea"]')
  //     .assistantGenerate('a simple storage contract', 'openai')
  //     .assistantGenerate('a simple storage contract', 'mistralai')
  //     .assistantGenerate('a simple storage contract', 'anthropic')
  //     .pause(10000)
  // },
  'Should create a new workspace using the AI assistant button in the composer': function (browser: NightwatchBrowser) {
    browser
      ...
      .setValue('*[data-id="modalDialogCustomPromptTextCreate"]', 'a simple storage contract')
