...
module.exports = {
  ...
  'open default prettier config and set the content': function (browser: NightwatchBrowser) {
    browser.openFile('.prettierrc.json').pause(1000).setEditorValue(JSON.stringify(defaultPrettierOptions, null, '\t'))
  },
  'Add Ballot': function (browser: NightwatchBrowser) {
    browser.addFile('Untitled.sol', { content: unformattedContract })
  },
  'Should put cursor at "unfomattedContract"': function (browser: NightwatchBrowser) {
    ...
    browser.waitForElementVisible('#editorView').click({
  ...
  'Format code': function (browser: NightwatchBrowser) {
    browser
      .perform(function () {
        ...
        return actions.keyDown(this.Keys.SHIFT).keyDown(this.Keys.ALT).sendKeys('f')
      })
      .pause(2000)
