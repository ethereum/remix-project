...
import { RunListInterface, TestCbInterface, TestResultInterface, ResultCbInterface, CompiledContract, AstNode, Options, FunctionDescription, UserDocumentation } from './types'
...
function getAvailableFunctions(fileAST: AstNode, testContractName: string): string[] {
  ...
  if (fileAST.nodes && fileAST.nodes.length > 0) {
    ...
    if (contractAST.length > 0 && contractAST[0].nodes) {
      const funcNodes: AstNode[] = contractAST[0].nodes.filter(
        (node) => (node.kind === 'function' && isNodeType(node, 'FunctionDefinition')) || isNodeType(node, 'FunctionDefinition')
      )
...
function getAssertMethodLocation(fileAST: AstNode, testContractName: string, functionName: string, assertMethod: string): string {
  if (fileAST.nodes?.length) {
    ...
    if (contractAST?.nodes?.length) {
      ...
      const assetExpression = expressions.find(
        (e) =>
...
export function runTest(
  testName: string,
  testObject: any,
  contractDetails: CompiledContract,
  fileAST: AstNode,
  opts: Options,
  testCallback: TestCbInterface,
  resultsCallback: ResultCbInterface
): void {
  let passingNum = 0
  let failureNum = 0
  let timePassed = 0
