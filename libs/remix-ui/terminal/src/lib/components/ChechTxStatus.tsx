...
const CheckTxStatus = ({tx, type}) => {
  if (tx.status === 1 || tx.status === '0x1' || tx.status === true) {
    return (
      <i className="remix_ui_terminal_txStatus remix_ui_terminal_succeeded fas fa-check-circle"></i>
    )
  }
  if (type === 'call' || type === 'unknownCall' || type === 'unknown') {
    ...
    return (
      <i className="remix_ui_terminal_txStatus remix_ui_terminal_failed fas fa-times-circle"></i>
    )
  } else {
    return (
      <i
