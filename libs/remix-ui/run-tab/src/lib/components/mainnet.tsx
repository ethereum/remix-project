...
export function MainnetPrompt(props: MainnetProps) {
  ...
  useEffect(() => {
    props.init((txFeeText, gasPriceValue, gasPriceStatus) => {
      ...
      if (
        props.network &&
        props.network.lastBlock &&
        props.network.lastBlock.baseFeePerGas
      ) {
        const baseFee = Web3.utils.fromWei(
          Web3.utils.toBN(props.network.lastBlock.baseFeePerGas),
          'Gwei'
        )
  ...
  const onMaxFeeChange = (value: string) => {
    ...
    if (
      Web3.utils
        .toBN(props.network.lastBlock.baseFeePerGas)
        .gt(Web3.utils.toBN(Web3.utils.toWei(maxFee, 'Gwei')))
    ) {
      setTransactionFee(
        'Transaction is invalid. Max fee should not be less than Base fee'
      )
  ...
  return (
    <div>
      <div className="text-dark">
        You are about to create a transaction on {props.network.name} Network.
        Confirm the details to send the info to your provider.
        <br />
        The provider for many users is MetaMask. The provider will ask you to
        sign the transaction before it is sent to {props.network.name} Network.
      </div>
      <div className="mt-3">
        <div>
        ...
        <div className="d-flex align-items-center">
          ...
          <pre className="udapp_wrapword mb-0">
            {props.tx.data && props.tx.data.length > 50
              ? props.tx.data.substring(0, 49) + '...'
              : props.tx.data}
        ...
        {props.network.lastBlock.baseFeePerGas ? (
          <div>
            ...
            >
              <div className="d-flex">
                ...
                />
                <span title="visit https://ethgasstation.info for current gas price info.">
                  Gwei
                </span>
              </div>
            </div>
            ...
            >
              <div className="d-flex">
                <span className="text-dark mr-2 text-nowrap">
                  Max fee (Not less than base fee{' '}
                  {Web3.utils.fromWei(
                    Web3.utils.toBN(props.network.lastBlock.baseFeePerGas),
                    'Gwei'
                  )}{' '}
          ...
          <div className="d-flex align-items-center my-1">
            ...
            <input
              ...
              id="gasprice"
              onInput={(e: any) => onGasPriceChange(e.target.value)}
            />
            <span>
              Gwei (visit{' '}
              <a
