...
import { createNewBlockchainAccount, setExecutionContext, signMessageWithAddress, callFaucetWithPlugin } from './account'
import {
  clearInstances,
  clearPopUp,
  removeInstance,
  setAccount,
  setGasFee,
  setMatchPassphrasePrompt,
  setNetworkNameFromProvider,
  setPassphrasePrompt,
  setSelectedContract,
  setSendTransactionValue,
  setUnit,
  updateBaseFeePerGas,
  updateConfirmSettings,
  updateGasPrice,
  updateGasPriceStatus,
  updateMaxFee,
  updateMaxPriorityFee,
  updateScenarioPath,
} from './actions'
import {
  createInstance,
  ...
  syncContractsInternal,
  isValidContractAddress,
  isValidContractUpgrade,
} from './deploy'
...
export const signMessage = (account: string, message: string, modalContent: (hash: string, data: string) => JSX.Element, passphrase?: string) =>
  signMessageWithAddress(plugin, dispatch, account, message, modalContent, passphrase)
...
) => createInstance(plugin, dispatch, selectedContract, gasEstimationPrompt, passphrasePrompt, publishToStorage, mainnetPrompt, isOverSizePrompt, args, deployMode)
...
) => runCurrentScenario(liveMode, plugin, dispatch, gasEstimationPrompt, passphrasePrompt, confirmDialogContent)
...
export const isValidProxyUpgrade = (proxyAddress: string, contractName: string, solcInput: SolcInput, solcOuput: SolcOutput) =>
  isValidContractUpgrade(plugin, proxyAddress, contractName, solcInput, solcOuput)
export const callFaucet = (providerId: string, recipient: string, value: number) => callFaucetWithPlugin(plugin, providerId, recipient, value)
