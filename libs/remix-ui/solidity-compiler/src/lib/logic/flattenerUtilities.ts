...
function _getDependencies(ast) {
  const dependencies = ast?.nodes.filter((node) => node?.nodeType === 'ImportDirective').map((node) => node?.file)
  return dependencies
}
...
Graph.prototype.sort = function () {
  ...
  function visit(node) {
    if (marks[node] === 'temp') throw new Error('There is a cycle in the graph. It is not possible to derive a topological sort.')
    else if (marks[node]) return
...
function resolve(parentPath, childPath) {
  ...
  const resolvedPath = resolvedParts.join('/').replace('http:/', 'http://').replace('https:/', 'https://')
  return resolvedPath
}
