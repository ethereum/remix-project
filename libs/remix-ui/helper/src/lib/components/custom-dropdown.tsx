...
export const CustomToggle = React.forwardRef(
  ({children, onClick, icon, className = ''}: {children: React.ReactNode; onClick: (e) => void; icon: string; className: string}, ref: Ref<HTMLButtonElement>) => (
    <button
      ref={ref}
      onClick={(e) => {
        e.preventDefault()
...
export const ProxyAddressToggle = React.forwardRef(
  (
    {address, onClick, className = '', onChange}: {address: string; onClick: (e) => void; className: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void},
    ref: Ref<HTMLDivElement>
  ) => (
    <div
      ref={ref}
